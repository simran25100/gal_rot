...
dfcygx <- read.csv("/Users/ssingh/Documents/PhD/R/cygx_all_full.CSV")
head(dfcygx)
names(dfcygx)
unique(dfcygx$cluster_name)
...
plot_cluster_zoom_cma <- function(cluster_names,
  ...
  par(mfrow = c(1,1), mar = c(4, 4, 1, 5), oma = c(0,0,0,5)) # mar = bottom, left, top, right
  cex <- 1.0
  
  # Select cluster stars
  cluster_stars <- dfcygx[dfcygx$cluster_name %in% cluster_names, ]
  if(nrow(cluster_stars) == 0) stop("No stars found for selected clusters.")
  
  # Define distance bins and symbols
  dist_breaks <- c(500, 900, 1300, 2000)
  dist_pch    <- c(0, 1, 2)
  
  # Assign pch based on distance
  dfcygx$pch_dist <- as.numeric(as.character(
    cut(dfcygx$D, breaks = dist_breaks, labels = dist_pch, include.lowest = TRUE)
  ))
  
  # Base plot: all other stars
  other_stars <- dfcygx[!dfcygx$cluster_name %in% cluster_names, ]
  ...
  cluster_colors <- c(
    "CygX South"        = "#E41A1C",  # red
    "IC 1318a"          = "#377EB8",  # blue
    "BD+40 4124"        = "#FF7F00",  # orange
    "Cyg X North"       = "#4DAF4A",  # green
    "IRAS 20231+3901"   = "#984EA3",  # purple
    "NGC 6914"          = "#F781BF",  # pink
    "Cyg X G79.1+0.9"   = "#A65628",  # brown
    "Cyg OB2"           = "#EFC000",  # yellow
    "DR 15"             = "#66C2A5",  # teal
    "IC 5070"           = "#FC8D62",  # salmon
    "LDN 896"           = "#8DA0CB",  # periwinkle
    "Cyg X G82.3+0.8"   = "#E78AC3"   # light purple
  )
  
  # Plot each cluster
  for(cl in cluster_names) {
    sub <- dfcygx[dfcygx$cluster_name == cl, ]
  ...
  legend(
    ...
    legend = c(cluster_names, "GAIA Data",
...
canis_clusters <- c("CygX South", "IC 1318a", "BD+40 4124", "Cyg X North", "IRAS 20231+3901", "NGC 6914", 
                    "Cyg X G79.1+0.9" ,"Cyg OB2", "DR 15" ,"IC 5070", "LDN 896", "Cyg X G82.3+0.8")
