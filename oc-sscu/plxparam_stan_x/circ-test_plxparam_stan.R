...
# Code to run 5k data SUBSET on the cluster with 10 chains  
# Using the plx paramater model. Eps and parallax are VECTORS with all other params = real

# - - - - -  - - - - -  - - - - -  - - - - -  - - - - -  - - - - -  - - - - - 
file_path <- "/beegfs/car/bxster25/oc-sscu/gaia18_subset/subset_24.csv" 
df <- fread(file_path)

# Subset to 100 samples
set.seed(123)  # Ensure reproducibility
df <- df[sample(nrow(df), 100), ]

options(mc.cores = 4)  #uses 4 cores (1 core per chain)
...
model {
  ...
  vector[N] f1_mean = (A * cos(2 * l) - 0 * sin(2 * l) + B) .* cos(b);
  vector[N] g1_mean = parallax .*(u0 * sin(l) - v0 * cos(l));
  
  vector[N] f2_mean = -(A * sin(2 * l) + 0 * cos(2 * l) + 0) .* sin(b) .* cos(b);
...
pars = c("A", "B", "u0", "v0", "w0", "s_l", "s_b", "nu_l", "nu_b")
print(fit, pars = pars)

timestamp <- format(Sys.time(), "%Y%m%d_%H%M%S")
filename <- paste0("/home2/bxster25/traceplots-sscu/ncirc/traceplot_circ_test_", timestamp, ".pdf")
pdf(filename, width = 8, height = 6)
print(traceplot(fit, pars = pars))
dev.off()

save(fit, file = "/beegfs/car/bxster25/oc-sscu/saveimage/circ_test.Rdata")
